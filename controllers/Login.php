...
class Login extends CI_Controller {
    ...
    public function index() {
        if ($this->session->userdata('admin_login')) {
            redirect(site_url('admin'), '1');
        }elseif ($this->session->userdata('user_login')) {
            redirect(site_url('user'), '1');
        }else {
            redirect(site_url('home/login'), '1');
        }
    }
    ...
    public function validate_login($from = "") {
        ...
        if ($query->num_rows() > 0) {
            ...
            if ($row->role_id == 1) {
                ...
                redirect(site_url('admin/dashboard'), '1');
            }else if($row->role_id == 2){
                $this->session->set_userdata('user_login', '1');
                redirect(site_url('home'), '1');
            }
        }else {
    ...
    public function register() {
        ...
        if(empty($data['first_name']) || empty($data['last_name']) || empty($data['email']) || empty($data['password'])){
            ...
            redirect(site_url('home/sign_up'), '1');
        }
        ...
        if($validity === 'unverified_user' || $validity == true) {
            ...
            if (get_settings('student_email_verification') == 'enable') {
                ...
                redirect(site_url('home/verification_code'), '1');
            }else {
